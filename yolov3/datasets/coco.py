...
class COCODataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 画像を読み込む。
        # MOT train test irekae
        # import os
        # if not os.path.isfile(img_path):
        #     if "test" in img_path:
        #         img_path = img_path.replace("test", "train")
        #     else:
        #         img_path = img_path.replace("train", "test")
        ...
        labels.resize(self.max_labels, 5, refcheck=False)
    ...
    def init_dataset(self, dataset_dir, anno_path):
        ...
        for img_id in self.coco.getImgIds():
            ...
            # if (dataset_dir / "ILSVRC2015_VID_train_0000"/img_name).exists():
            #     img_path = str(dataset_dir / "ILSVRC2015_VID_train_0000"/img_name)
            # else:
            #     img_path = str(dataset_dir /img_name)
            #     img_path = str(dataset_dir / "val" / img_name)

            # img_path = str(dataset_dir /img_name)
            # if (dataset_dir / "train" / img_name).exists():
            #     img_path = str(dataset_dir / "train" / img_name)
            # if (dataset_dir / "test" / img_name).exists():
            #     img_path = str(dataset_dir / "train" / img_name)
            ...
            # fixed = ["MOT17-02", "MOT17-04", "MOT17-09", "MOT17-01", "MOT17-03", "MOT17-08"]
            # for f in fixed:
            #     if f in img_path:
